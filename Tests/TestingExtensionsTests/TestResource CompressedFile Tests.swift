...
@Suite
struct TestResource_CompressedFile_Tests_bar {
    @Test
    func compressedResourceData_deflate() throws {
    ...
    @Test
    func compressedResourceData_lz4() throws {
        #if canImport(Darwin) // lz4 is not yet supported on non-Apple platforms
        let data = try TestResource.bar(.lz4).data()
        let string = try #require(String(data: data, encoding: .utf8))
        #expect(string == "Bar file content")
        #else
        withKnownIssue("LZ4 Compression is only supported on Apple platforms") {
            _ = try TestResource.bar(.lz4).data()
        }
        #endif
    }
    ...
    @Test
    func compressedResourceData_lzfse() throws {
        #if canImport(Darwin) // lzfse is not yet supported on non-Apple platforms
        let data = try TestResource.bar(.lzfse).data()
        let string = try #require(String(data: data, encoding: .utf8))
        #expect(string == "Bar file content")
        #else
        withKnownIssue("LZFSE Compression is only supported on Apple platforms") {
    ...
    @Test
    func compressedResourceData_lzma2xz() throws {
...
@Suite
struct TestResource_CompressedFile_Tests_baz {
    ...
    @Test
    mutating func compressedResourceData_deflate() throws {
    ...
    @Test
    mutating func compressedResourceData_lz4() throws {
        #if canImport(Darwin) // lz4 is not yet supported on non-Apple platforms
        let data = try TestResource.baz(.lz4).data()
        #expect(data == expectedData)
        #else
        withKnownIssue("LZ4 Compression is only supported on Apple platforms") {
            _ = try TestResource.baz(.lz4).data()
        }
        #endif
    }
    ...
    @Test
    mutating func compressedResourceData_lzfse() throws {
        #if canImport(Darwin) // lzfse is not yet supported on non-Apple platforms
        let data = try TestResource.baz(.lzfse).data()
        #expect(data == expectedData)
        #else
        withKnownIssue("LZFSE Compression is only supported on Apple platforms") {
            _ = try TestResource.baz(.lzfse).data()
        }
        #endif
    }
    ...
    @Test
    mutating func compressedResourceData_lzma2xz() throws {
